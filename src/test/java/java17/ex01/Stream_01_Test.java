...
//		pizzas.stream().filter(pizza -> pizza.getPrice() >= 1300);
	...
	public void test_stream_anyMatch() throws Exception {
		...
		Boolean result2 = !pizzas.stream()
				.filter(pizza -> pizza.getPrice() >= 2000)
				.toList()
				.isEmpty();
	...
	public void test_stream_allMatch() throws Exception {
		...
		Boolean result2 = pizzas.stream()
				.filter(pizza -> pizza.getPrice() < 900)
				.toList()
				.isEmpty();
	...
	public void test_stream_filter_adv() throws Exception {
		...
		List<Order> result = orders.stream()
				.filter(order -> order.getPizzas().stream()
						.filter(pizza -> !pizza.getName().equals("Pépéroni")).toList().isEmpty())
				.toList();
